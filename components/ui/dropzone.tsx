...
interface DropZoneAreaProps extends React.ButtonHTMLAttributes<HTMLButtonElement> { }
const DropZoneArea = forwardRef<HTMLButtonElement, DropZoneAreaProps>(
  ({ className, children, ...props }, forwardedRef) => {
    ...
    return (
      ...
      <button
        // Use mergeRefs to properly forward both refs
        ref={(node) => {
          if (typeof forwardedRef === "function") {
            forwardedRef(node);
          } else if (forwardedRef && typeof forwardedRef === "object" && forwardedRef !== null) {
            (forwardedRef as React.MutableRefObject<HTMLButtonElement | null>).current = node;
          }
          // Forward the dropzone ref if present
          if (context.getRootProps().ref) {
            if (typeof context.getRootProps().ref === "function") {
              context.getRootProps().ref(node);
            } else if (context.getRootProps().ref) {
              if (node !== null) {
                (context.getRootProps().ref as React.RefObject<HTMLButtonElement>).current = node;
              }
            }
          }
        }}
...
const InfiniteProgress = forwardRef<HTMLProgressElement, InfiniteProgressProps>(
  ({ className, status, ...rest }, ref) => {
    const done = status === "success" || status === "error";
    const error = status === "error";

    // Only pick allowed progress attributes from rest
    const { id, style, title, ..._ } = rest;

    return (
      <progress
        ref={ref}
        value={status === "success" ? 100 : status === "error" ? 0 : undefined}
        max={100}
      ...
      >
        {/* Fallback for browsers that don't support <progress> */}
        <div
          className={cn(
