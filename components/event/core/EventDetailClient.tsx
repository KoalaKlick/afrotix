...
export function EventDetailClient({
  ...
  const showMembers =
    event.type === "standard" || event.votingMode === "internal";
  ...
  async function handleFlierUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (result.success && result.data) {
        setFormData((prev: EventFormData) => ({
          ...prev,
          flierImage: result.data.path,
        }));
        await saveField("flierImage", result.data.path);
        toast.success("Flier image uploaded!");
      } else {
        toast.error(
          result.success ? "Failed to upload flier image" : result.error,
        );
      }
    } catch {
        ...
        <AlertDialogContent variant="afro">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <em className="text-center bg-secondary-100/50">
                {" "}
                <br /> Note: No amount is charged from your account.
              </em>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
