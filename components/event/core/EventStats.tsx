...
function getIconColorStyles(iconSrc: string): {
  ...
  const colorGlow: Record<string, string> = {
    ...
    yellow:
      "radial-gradient(circle at bottom right,color-mix(in srgb, var(--color-secondary-500) 28%, transparent),transparent 50%)",
    green:
      "radial-gradient(circle at bottom right,color-mix(in srgb, var(--color-primary-600) 28%, transparent),transparent 50%)",
    black:
      "radial-gradient(circle at bottom right,rgba(160,160,160,0.18),transparent 50%)",
  };
  ...
  const border =
    color && borderColor[color] ? borderColor[color] : "border-white/10";
...
export function StatCard({
  ...
  const variantStyles = {
    default: "bg-card",
    success:
      "bg-tertiary-50 dark:bg-primary-950/20 border-primary-200 dark:border-primary-900",
    warning:
      "bg-secondary-50 dark:bg-secondary-950/20 border-secondary-200 dark:border-secondary-900",
    danger:
      "bg-tertiary-50 dark:bg-tertiary-950/20 border-tertiary-200 dark:border-tertiary-900",
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p
                className={cn(
                  sizeClass,
                  "font-bold font-montserrat text-black",
                )}
              >
                {display}
              </p>
            ...
            {iconSrc && (
              <NextImage
                ...
                width={100}
                height={100}
                className="h-full group-hover:-bottom-7 duration-300 group-hover:opacity-70 transition-all  select-none w-auto object-cover opacity-20 absolute -bottom-8 -right-8"
              />
            )}
            {!iconSrc && Icon && (
