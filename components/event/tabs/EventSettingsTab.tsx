...
export function EventSettingsTab({
  ...
  const [selectedSponsor, setSelectedSponsor] = useState<EventSponsor | null>(
    null,
  );

  const [socialModalOpen, setSocialModalOpen] = useState(false);
  const [selectedSocial, setSelectedSocial] = useState<
    NonNullable<EventFormData["socialLinks"]>[number] | null
  >(null);

  const [galleryModalOpen, setGalleryModalOpen] = useState(false);
  const [selectedGallery, setSelectedGallery] = useState<
    NonNullable<EventFormData["galleryLinks"]>[number] | null
  >(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-md p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Showcase organizations supporting your event
            </p>
          </div>
          {canEdit && (
            <Button
              ...
              disabled={
                isPending || (formData.sponsors?.length ?? 0) >= MAX_SPONSORS
              }
            >
        ...
        <div className="flex gap-4 flex-wrap">
          {(formData.sponsors ?? []).map((sponsor) => (
            <div
              key={sponsor.id}
              className="group w-fit relative p-4 rounded-md border bg-card hover:border-primary/50 transition-all flex flex-col items-center gap-4 text-center"
            >
      ...
      <Card className="rounded-md p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Share links to event photos (Google Drive, Pixieset, etc.)
            </p>
          </div>
          {canEdit && (
            <Button
              ...
              disabled={
                isPending ||
                (formData.galleryLinks?.length ?? 0) >= MAX_GALLERY_LINKS
              }
            >
              <Plus className="size-4 mr-2" />
              Add Gallery
                ...
                {canEdit && (
                  <div className="flex gap-1 ml-auto">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        const newLinks = (formData.galleryLinks ?? []).filter(
                          (l) => l.id !== link.id,
                        );
                        setFormData((prev) => ({
                          ...prev,
                          galleryLinks: newLinks,
                        }));
        ...
        onSave={(sponsorData) => {
          ...
          if (selectedSponsor) {
            newSponsors = (formData.sponsors ?? []).map((s) =>
              s.id === selectedSponsor.id ? { ...sponsorData, id: s.id } : s,
            );
          } else {
            newSponsors = [
              ...(formData.sponsors ?? []),
              { ...sponsorData, id: `new-${Date.now()}` },
            ];
          }
      ...
      <SocialLinkDialog
        ...
        link={
          selectedSocial
            ? { id: String(selectedSocial.id ?? ""), url: selectedSocial.url }
            : null
        }
        onSave={(url) => {
          ...
          if (selectedSocial) {
            newLinks = (formData.socialLinks ?? []).map((l) =>
              l.id === selectedSocial?.id ? { id: l.id, url } : l,
            );
          } else {
            newLinks = [
      ...
      <GalleryLinkDialog
        ...
        onSave={(galleryData) => {
          ...
          if (selectedGallery) {
            newLinks = (formData.galleryLinks ?? []).map((l) =>
              l.id === selectedGallery?.id ? { ...galleryData, id: l.id } : l,
            );
          } else {
            newLinks = [
              ...(formData.galleryLinks ?? []),
              { ...galleryData, id: `new-${Date.now()}` },
            ];
          }
