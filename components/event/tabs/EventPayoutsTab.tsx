...
export function EventPayoutsTab({
  ...
  const revenueBreakdownCards = useMemo(() => {
    const hasRevenue =
    ...
    const ticketCard = isTicketed
      ? {
          label: "Ticket Revenue",
          value: formatAmount(eventStats.ticketRevenue),
          subLabel: `${eventStats.totalOrders.toLocaleString()} ${eventStats.totalOrders === 1 ? "order" : "orders"}`,
          icon: Ticket,
          accent: event.organization.secondaryColor || "#CE1126",
        }
      : null;

    const voteCard = isVoting
      ? {
          label: "Vote Revenue",
          value: formatAmount(eventStats.voteRevenue),
          subLabel: `${eventStats.totalVotes.toLocaleString()} ${eventStats.totalVotes === 1 ? "vote" : "votes"}`,
          icon: Vote,
    ...
    const nominationCard =
      isVoting && eventStats.nominationRevenue > 0
        ? {
            label: "Nomination Revenue",
            value: formatAmount(eventStats.nominationRevenue),
            subLabel: `${eventStats.totalNominees.toLocaleString()} ${eventStats.totalNominees === 1 ? "nominee" : "nominees"}`,
            icon: Vote,
            accent: "#7C3AED",
          }
        : null;

    return [ticketCard, voteCard, nominationCard].filter(Boolean);
  }, [
    event.organization.secondaryColor,
    eventStats.ticketRevenue,
    eventStats.voteRevenue,
    eventStats.nominationRevenue,
    eventStats.totalOrders,
    eventStats.totalVotes,
    eventStats.totalNominees,
    isTicketed,
    isVoting,
  ]);

  const fallbackCountCards = useMemo(
    () => [
      {
        label: "Ticket Orders",
  ...
  const summaryCards = useMemo(
    () => [
      ...
      ...(revenueBreakdownCards ?? fallbackCountCards),
    ],
    [
  ...
  async function refreshTicketTransactions() {
    ...
    try {
      const result = await getEventTicketTransactionsAction(event.id, {
        limit: 10,
      });
  ...
  async function refreshVoteTransactions() {
    ...
    try {
      const result = await getEventVoteTransactionsAction(event.id, {
        page: 1,
        limit: 10,
      });
