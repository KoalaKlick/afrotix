...
import { RegistrationFieldManager } from "../settings/RegistrationFieldManager";
...
export function MemberManager({
  ...
  const columns: Column<Member>[] = [
    ...
    {
      header: "Status",
      cell: (member) => <StatusBadge variant={member.status} />,
    },
    {
      header: "Registered",
      cell: (member) => (
      ...
      cell: (member) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 rounded-xl">
            <DropdownMenuLabel className="text-[10px] font-bold  tracking-widest">
              Actions
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem
    ...
    <div className="space-y-6">
      ...
      <div className="bg-muted/30 border p-6 rounded-md flex items-start gap-4">
        ...
        <div className="space-y-1">
          <p className="font-black uppercase tracking-tight text-sm text-muted-foreground">
            About Participants
          </p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Add members manually, import them in bulk, or define custom
            registration fields. Each member receives a unique code for
            verification.
          </p>
        </div>
      </div>
