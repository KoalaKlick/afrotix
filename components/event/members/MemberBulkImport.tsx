...
export function MemberBulkImport({
    ...
    const newMembers: ParsedMember[] = rows.map((row) => {
      ...
      headers.forEach((header, index) => {
        ...
        if (header === "full name" || header === "name") {
          ...
          const field = registrationFields.find(
            (f) => f.label.toLowerCase() === header,
          );
          if (field) {
  ...
  const handleImport = () => {
    ...
    startTransition(async () => {
      ...
      const result = await bulkAddEventMembersAction(
        eventId,
        membersToImport as any,
      );
      if (result.success) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent
        side="right"
        variant="afro"
        className="sm:max-w-2xl w-full p-0 flex flex-col h-full"
      >
        <SheetHeader className="p-6 border-b shrink-0">
          <div className="flex items-center justify-between">
            <div>
              ...
              <SheetDescription>
                Download the template, fill it out, and upload it back.
              </SheetDescription>
            </div>
            <Button
              variant="outline"
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                  Upload a CSV file containing your member list. Use our
                ...
                <Table>
                  ...
                  <TableBody>
                    {parsedData.map((member, i) => (
                      <TableRow
                        key={i}
                        className={cn(!member.isValid && "bg-destructive/5")}
                      >
