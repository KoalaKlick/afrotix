...
export function PublicNominationModal({
    ...
    startTransition(async () => {
      ...
      if (pendingFile) {
        ...
        if (!user) {
          ...
          if (authError) {
            toast.error(
              "Unable to establish a secure session for upload. Please try again.",
            );
            return;
          }
        }
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      const origin =
        process.env.NEXT_PUBLIC_DOMAIN_URL || globalThis.location?.origin || "";
      ...
      if (!response?.accessCode) {
        throw new Error(
          response?.error ?? response?.detail ?? "Failed to get access code",
        );
      }
  ...
  return (
    <>
      ...
      <Sheet open={sheetOpen} onOpenChange={handleSheetOpenChange}>
        ...
        <SheetContent
          side="right"
          className="sm:max-w-xl w-full flex flex-col h-full p-0 sm:p-6"
          variant="afro"
        >
          <SheetHeader className="shrink-0 pt-6 px-6 sm:p-0">
            <SheetTitle>Nominate Someone</SheetTitle>
            <SheetDescription>
              Submit a nomination for {category.name}. It will be reviewed by
              the organizers before appearing publicly.
              {isPaid && (
          ...
          <form
            onSubmit={onSubmit}
            className="flex-1 overflow-y-auto space-y-6 pt-4 px-6 sm:px-1 min-h-0"
          >
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, optionText: e.target.value }))
                  }
                  required
                />
              </div>
              ...
              {category.customFields && category.customFields.length > 0 && (
                <div className="space-y-4 pt-2 border-t">
                  ...
                  {category.customFields.map((field) => {
                    const fieldValue = form.fieldValues.find(
      ...
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {isPaid ? "Confirm & Pay" : "Confirm Nomination"}
            </DialogTitle>
          </DialogHeader>
                ...
                {isPaid ? (
                  ...
                  >
                    {payLoading ? (
                      ...
                      <>
                        <CreditCard className="size-4 mr-2" /> Confirm &amp; Pay
                        GHS {nominationPrice.toFixed(2)}
                      </>
                    )}
                  </Button>
              ...
              <div className="flex flex-col items-center py-10 space-y-4 text-center">
                ...
                <div>
                  <p className="font-black text-xl">
                    {isPaid ? "Payment failed" : "Submission failed"}
                  </p>
