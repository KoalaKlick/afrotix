...
export function CategoryList({
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <TabsList
              variant="afro"
              className={cn("w-full justify-start overflow-y overflow-x-auto")}
            >
      ...
      {categories.map((category) => {
        const pendingCount = category.votingOptions.filter(
          (o) => o.status === "pending",
        ).length;
        const approvedNominees = category.votingOptions.filter(
          (o) => o.status === "approved",
        );
                ...
                <div className="space-y-1 mt-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    {Number(category.votePrice) > 0 && (
                      <Badge
                        variant="secondary"
                        className="text-xs bg-[#009A44]/10 text-[#009A44] border-[#009A44]/20"
                      >
                    ...
                    {Number(category.nominationPrice) > 0 && (
                      <Badge
                        variant="secondary"
                        className="text-xs bg-[#D21034]/10 text-[#D21034] border-[#D21034]/20"
                      >
                        GHS {Number(category.nominationPrice).toFixed(2)} /
                        nominee
                      </Badge>
                    )}
                    {pendingCount > 0 && (
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will delete "{category.name}" and all its
              ...
              <div className="grid @md:grid-cols-2 @2xl:grid-cols-3 @5xl:grid-cols-4 @7xl:grid-cols-5 gap-4">
                {approvedNominees.map((option) => (
                  <NomineeCard
                    ...
                    onDelete={(code?: string) => code && onDeleteOption(option.id, code)}
