...
import { Image } from "@/components/shared/image/avatar";
...
export function EventStep4Extras({
  ...
  const updateSponsor = (
    ...
    if (field === "logo") {
      ...
      newSponsors[index] = {
        ...newSponsors[index],
        [field]: value,
        logo: newSponsors[index].logo ?? null,
      };
    }
  ...
  const updateGalleryLink = (
    index: number,
    field: keyof GalleryLink,
    value: string,
  ) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onSuccess({
      ...
      galleryLinks: galleryLinks.filter(
        (g) => g.name.trim() !== "" && g.url.trim() !== "",
      ),
    });
  };
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Add event sponsors to showcase their logos
            </p>
          </div>
          <Button
            type="button"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {sponsors.map((sponsor, index) => (
            <div
              key={index}
              className="relative p-4 rounded-xl border bg-card flex items-start gap-4"
            >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Dynamic links to event's social media profiles
            </p>
          </div>
          <Button
            type="button"
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {galleryLinks.map((link, index) => (
            <div
              key={index}
              className="grid grid-cols-1 sm:grid-cols-[1fr_2fr_auto] gap-3 items-end p-4 rounded-xl border bg-primary/5"
            >
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={link.name}
                  onChange={(e) =>
                    updateGalleryLink(index, "name", e.target.value)
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={link.url}
                  onChange={(e) =>
                    updateGalleryLink(index, "url", e.target.value)
                  }
                />
              </div>
              <Button
