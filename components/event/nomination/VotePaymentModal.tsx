...
import {
  revalidatePublicVoting,
  checkVoteStatusAction,
  castMemberVote,
...
export function VotePaymentModal({
  ...
  const checkVoteStatus = useCallback(async () => {
    ...
    try {
      const result = await checkVoteStatusAction(
        categoryId,
        eventId,
        uniqueCode,
      );
  ...
  const handleSubmitPayment = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        // FunctionsHttpError.message is always the generic Supabase wrapper text.
      ...
      if (response?.accessCode) {
        ...
        throw new Error(
          response?.error ||
            response?.detail ||
            "Failed to get access code from Paystack",
        );
      }
    } catch (err: any) {
                  ...
                  <>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-[10px] text-muted-foreground pl-1">
                        Check your email for the 8-character code sent by the
                        organizer
                      </p>
                    </div>
              ...
              <div className="p-10 flex flex-col items-center text-center space-y-6 animate-in fade-in zoom-in duration-300">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground max-w-70">
                    ...
                    for{" "}
                    <span className="text-[#009A44] font-bold">
                      {nominee.optionText}
                    </span>{" "}
                    {isInternalVoting ? "has" : "have"} been recorded
                    successfully.
                  </p>
                </div>
                <Button
