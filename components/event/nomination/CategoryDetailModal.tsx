...
export function CategoryDetailModal({
  ...
  const sorted = [...category.votingOptions].sort(
  ...
  const data = sorted.map((opt, i) => ({
    name:
      opt.optionText.length > 12
        ? `${opt.optionText.slice(0, 11)}…`
        : opt.optionText,
    fullName: opt.optionText,
    votes: opt.votesCount,
    fill: BAR_COLORS[i % BAR_COLORS.length],
    pct:
      totalVotes > 0 ? ((opt.votesCount / totalVotes) * 100).toFixed(1) : "0",
  }));

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg sm:h-auto  h-screen border-primary-900/10 overflow-hidden">
        <DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-tertiary-900/10 overflow-hidden">
            <div
              ...
              style={{
                backgroundImage:
                  "radial-gradient(circle at top right, color-mix(in srgb, var(--color-tertiary-600) 10%, transparent), transparent 60%)",
              }}
            >
              <span>#</span>
            ...
            <div className="divide-y max-h-60 overflow-y-auto overflow-x-auto">
              {sorted.map((opt, i) => {
                ...
                return (
                  <div
                    key={opt.id}
                    className="grid grid-cols-[auto_auto_1fr_auto_auto] gap-x-3 px-4 py-2.5 text-sm items-center"
                  >
                    ...
                    <span className="text-right font-medium tabular-nums">
                      {opt.votesCount.toLocaleString()}
                    </span>
                    <span className="text-right text-muted-foreground tabular-nums w-14">
                      {pct}%
                    </span>
                  </div>
                );
