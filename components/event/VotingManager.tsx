...
import { Plus, Trash2, Pencil, Loader2, Vote } from "lucide-react";
...
export function VotingManager({
  ...
  const [editingCategory, setEditingCategory] = useState<VotingCategory | null>(
    null,
  );

  // Custom fields dialog state
  const [fieldsDialogOpen, setFieldsDialogOpen] = useState(false);
  const [fieldsCategory, setFieldsCategory] = useState<VotingCategory | null>(
    null,
  );
  ...
  function handleSaveField() {
    ...
    const fieldName =
      fieldForm.fieldName.trim() ||
      fieldForm.fieldLabel.toLowerCase().replaceAll(/\s+/g, "_");

    startTransition(async () => {
      if (editingField && fieldsCategory) {
        const result = await updateCategoryField(
          editingField.id,
          fieldsCategory.id,
          {
            fieldName,
            ...
            options:
              fieldForm.fieldType === "select" && fieldForm.options.trim()
        ...
        setCategories((prev) =>
          prev.map((c) => ({
            ...
            votingOptions: c.votingOptions.map((o) =>
              o.id === optionId
                ? { ...o, status: "approved" as VotingOptionStatus }
                : o,
            ),
          })),
        );
      ...
      if (result.success) {
        setCategories((prev) =>
          prev.map((c) => ({
            ...
            votingOptions: c.votingOptions.map((o) =>
              o.id === optionId
                ? { ...o, status: "rejected" as VotingOptionStatus }
                : o,
            ),
          })),
        );
          ...
          {canEdit &&
            (() => {
              const pendingCount = categories.reduce(
                (sum, cat) =>
                  sum +
                  cat.votingOptions.filter((o) => o.status === "pending")
                    .length,
                0,
              );
      ...
      <OptionSheet
        ...
        onOptionCreated={(catId, newOption) => {
          setCategories((prev) =>
            prev.map((c) =>
              c.id === catId
                ? { ...c, votingOptions: [...c.votingOptions, newOption] }
                : c,
            ),
          );
        }}
      ...
      >
        ...
        >
          <SheetHeader className="shrink-0">
            <SheetTitle>Custom Fields for {fieldsCategory?.name}</SheetTitle>
            <SheetDescription>
          ...
          <SheetBody className="space-y-4 flex-1 overflow-y-auto pr-2">
            {/* Existing Fields */}
            {fieldsCategory &&
              fieldsCategory.customFields &&
              fieldsCategory.customFields.length > 0 && (
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  ...
                                  {field.fieldLabel}&quot; and all its data.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
